// SmartFitApp.java  — Pure JavaFX, no Scene Builder, no FXML
//
// ALL original class files (ClothingItems, Outfit, OutfitManager,
// OutfitScheduler, Wardrobe) are used UNCHANGED.
//
// Screens:
//   1. Home              – gradient bg, SmartFit title, two buttons
//   2. Add Item          – photo upload + fields per category
//   3. Wardrobe / Build  – LEFT: scrollable item grid (like screenshot)
//                          RIGHT: live outfit preview panel
//                          + Save Outfit, Outerwear & Accessories buttons
//   4. Outerwear & Acc   – same grid layout for those two categories
//   5. Try Combinations  – THE FIT preview + suggestions + rating
//   6. Outfit Scheduler  – month calendar, click day → assign outfit
//   7. Online Search     – drag & drop image upload
//
// COMPILE (all .java in same folder):
//   javac --module-path "<fx>/lib" --add-modules javafx.controls,javafx.graphics *.java
// RUN:
//   java  --module-path "<fx>/lib" --add-modules javafx.controls,javafx.graphics -Dprism.order=sw SmartFitApp
...
public class App extends Application {
    ...
    private static final String BG_IMAGE_PATH = "C:\\Users\\hp\\Desktop\\javGUI\\SmartFit\\bg.png"; // ← PUT YOUR PATH HERE

    private static final Color  C_PINK    = Color.web("#F4A7B9"); // left edge
    private static final Color  C_MID     = Color.web("#C084FC"); // center
    private static final Color  C_PURPLE  = Color.web("#7C3AED"); // right edge

    private static final String BTN_FILL      = "#E06FA0"; // coral rose
    private static final String BTN_HOV       = "#C8527E"; // darker on hover
    private static final String BTN_CORAL     = "#F4845F"; // warm coral accent
    ...
    // Photo map: item → image URL
    private final Map<ClothingItems, String> imgMap = new HashMap<>();

    // Search / reference upload URLs
    private String searchUrl = null;
    private String refTopUrl = null;
    private String refBotUrl = null;

    // Root pane (screens swap inside this)
    private StackPane root;

    // Live grid rows (rebuilt on wardrobe change)
    ...
    // Scheduler calendar reference month
    ...
    public void start(Stage stage) {
        ...
        stage.setMinWidth(940); stage.setMinHeight(600);
        stage.show();
    }
    ...
    private void showAddItem() {
        ...
        // Default extras for Top
        HBox defExtra = row2(node("Style", styleF), node("Sleeve", sleeveCB));
        extraBox.getChildren().add(defExtra);
        ...
        catGrp.selectedToggleProperty().addListener((o, old, sel) -> {
            ...
            if (cat.equals("Shoes")) { er.getChildren().add(node("Heel cm", heelF)); er.getChildren().add(openCB); }
        ...
        Label msgLbl = new Label(""); msgLbl.setStyle("-fx-font-size:12px;-fx-text-fill:#FFD700;");
        ...
        addBtn.setOnAction(ev -> {
            // ── All fields have defaults — nothing is required ─────────────
            int price = 0;
            try { price = Integer.parseInt(priceF.getText().trim()); }
            catch (Exception ex) { price = 0; } // default price = 0
            ...
            brandF.clear(); sizeF.clear(); colourF.clear(); priceF.clear(); fabricF.clear(); heelF.clear();
    ...
    // Left: scrollable item grid (like the screenshot — 3 columns)
    // Right: live outfit preview panel
    // ═════════════════════════════════════════════════════════════════════
    private void showWardrobe() {
        ...
        saveBtn.setOnAction(e -> {
            ...
            String name = nameF.getText().isBlank()
                    ? "Outfit " + (outfitMgr.getTotalOutfits()+1) : nameF.getText().trim();
    ...
    // ── Live outfit preview panel (right side) ────────────────────────────
    // Fixed slots stacked vertically like a mannequin:
    //   TOP     slot — 220 × 200 px
    //   BOTTOM  slot — 220 × 200 px
    //   SHOES   slot — 220 × 130 px
    //   OUTER   slot — 220 × 160 px  (hidden until outerwear selected)
    //   ACC     slot — 220 ×  80 px  (hidden until accessory selected)
    private VBox buildPreviewPanel() {
        VBox panel = new VBox(0); // slots sit flush, no gap — images touch like a mannequin
    ...
    private void refreshPreviewInto(VBox panel) {
        ...
        // ── Wardrobe panel only shows the 3 core slots ───────────────────
        // Outerwear & Accessories appear in the Outerwear screen full view
        //
        // Slot dimensions:
        //   Top    → 220 × 200 px
        //   Bottom → 220 × 200 px
    ...
    /**
     * One fixed-height slot in the preview panel.
     * If item is null → shows a labelled empty placeholder box.
     * If item has a photo → shows the full photo filling the slot.
     * If item has no photo → shows a large colour swatch + emoji.
     *
     * @param label     category label shown in placeholder
     * @param item      the selected ClothingItem, or null
     * @param type      "top"/"bottom"/"shoes"/"outer"/"acc" (for emoji fallback)
     * @param slotW     slot width  in px  (always 220)
     * @param slotH     slot height in px
     */
    private StackPane previewSlot(String label, ClothingItems item, String type,
        ...
        if (item == null) {
            ...
            if (url != null) {
                ...
                // ── No photo: large colour swatch + emoji ──────────────────
    ...
    // LEFT:  full outfit flat-lay (like image 3) — all 5 slots arranged
    //        in a styled grid: outerwear top-left, top centre, acc top-right
    //                          bottom centre, shoes bottom-left
    // RIGHT: outerwear + accessories selection grid + rating + suggestions
    // ═════════════════════════════════════════════════════════════════════
    private void showOuterAcc() {
        ...
        // ── LEFT — Full outfit flat-lay ───────────────────────────────────
        // Arranged like image 3:  a 3-column × 3-row grid
        //   [outerwear]  [  top  ]  [accessory]
        //   [  empty  ]  [bottom ]  [  empty  ]
        //   [ shoes   ]  [  empty]  [  empty  ]
        //
        // Slot sizes — increased for better visibility:
        //   outerwear  200 × 200   top    200 × 200   accessory 150 × 150
        //   bottom     200 × 210   shoes  200 × 160
        ...
        // ── RIGHT — selection grid + suggestions + rating ─────────────────
        ...
        // Refresh flat-lay when user clicks items in the grid
        // We do this by re-calling showOuterAcc after each selection
        // — handled in itemCard via refreshOuterAccFlatlay helper below
        ...
        saveOutfitBtn.setOnAction(e -> {
            ...
            Outfit last = all.get(all.size()-1);
            last.setRating(ratingVal[0]);
            alert("✅  Rating saved for outfit '" + last.getName() + "'!");
        });

        Button doneBtn = pill("← Back to Wardrobe", 220, 46);
        doneBtn.setOnAction(e -> { refreshPreview(); fade(this::showWardrobe); });

        VBox rightCol = new VBox(12, outerGrid, accGrid, sugCard, ratingCard,
                new HBox(12, saveOutfitBtn, doneBtn));
        ((HBox) rightCol.getChildren().get(rightCol.getChildren().size()-1)).setAlignment(Pos.CENTER_LEFT);
    ...
    /**
     * A flat-lay slot — smaller version of previewSlot, used in the
     * Outerwear & Accessories full-outfit view.
     */
    private StackPane flatSlot(ClothingItems item, String type,
        ...
        if (item == null) {
            slot.setStyle("-fx-background-color:rgba(255,255,255,0.06);"
                    ...
                    + "-fx-border-style:dashed;-fx-border-width:1.2;-fx-background-radius:10;"
                    + "-fx-border-radius:10;");
            Label ph = new Label(label);
            ph.setStyle("-fx-font-size:10px;-fx-text-fill:" + TXT_DIM + ";-fx-font-weight:bold;");
            ph.setWrapText(true); ph.setTextAlignment(javafx.scene.text.TextAlignment.CENTER);
            ph.setMaxWidth(w - 10);
            slot.getChildren().add(ph);
        } else {
            String url = imgMap.get(item);
    ...
    private void showTryCombinations() {
        ...
        // Show uploaded reference images or item summary
        ...
        if (hasImg) {
            ...
            // Text summary of selected items
        ...
        // Right — suggestions + rating + buttons
        VBox sugCard = card(370, 160);
        sugCard.getChildren().addAll(cardSec("SUGGESTIONS:"),
                wrapLabel(buildSuggestion()));
    ...
    // ═════════════════════════════════════════════════════════════════════
    // SCREEN 6 — OUTFIT SCHEDULER (month calendar)
    // ═════════════════════════════════════════════════════════════════════
    private void showScheduler() {
        ...
        // Calendar grid — rebuild on month change
        ...
        Runnable buildCal = () -> {
            ...
            monthLbl.setText(calMonth.getMonth().getDisplayName(TextStyle.FULL, Locale.ENGLISH)
                    + "  " + calMonth.getYear());

            String[] dayNames = {"Sun","Mon","Tue","Wed","Thu","Fri","Sat"};
            for (int d = 0; d < 7; d++) {
                Label dl = new Label(dayNames[d]);
                dl.setStyle("-fx-font-size:10px;-fx-font-weight:bold;-fx-text-fill:"
                        + TXT_DIM + ";");
            ...
            for (int day = 1; day <= daysInMonth; day++) {
                ...
                // Click → assign / unassign outfit popup
    ...
    // Popup when user clicks a calendar day
    private void showDayPopup(LocalDate date, GridPane calGrid) {
        ...
        // Dropdown to add a saved outfit
...
assignBtn.setOnAction(e -> {
    ...
    private void showOnlineSearch() {
        ...
        zone.setOnMouseClicked(e -> {
            File f = pickImg(); if (f == null) return;
            searchUrl = f.toURI().toString(); showInDrop(prev, hint, searchUrl);
        });
        dnd(zone, prev, hint, u -> searchUrl = u);

        Button startBtn = pill("START SEARCHING", 260, 52);
        startBtn.setOnAction(e -> {
            if (searchUrl != null) fade(this::showOuterAcc); else shake(zone);
        });
    ...
    private void refreshGrid() {
        ...
        if (rowTops .getChildren().isEmpty()) rowTops .getChildren().add(emptyLbl("No tops yet"));
    ...
    // ═════════════════════════════════════════════════════════════════════
    // ITEM CARD (used in wardrobe grid)
    // ═════════════════════════════════════════════════════════════════════
    private VBox itemCard(ClothingItems item, String type) {
        ...
        card.setOnMouseClicked(e -> {
            ...
            // For outer/acc: reload the whole screen so the flat-lay updates
    ...
    // One scrollable row in the wardrobe grid
    ...
    private void seedSampleData() {
        // Wardrobe starts empty — user adds their own items via Add Clothing screen
    }
    ...
    /** Background: loads BG_IMAGE_PATH if set, otherwise uses the gradient fallback */
    private StackPane grad() {
        ...
        if (!BG_IMAGE_PATH.isEmpty()) {
            ...
            try {
                ...
                // Path invalid — fall back to gradient silently
                addGradRect(pane);
            }
        } else {
            // ── Gradient fallback (used until you set BG_IMAGE_PATH) ──────
            addGradRect(pane);
        }
        return pane;
    }
    ...
    private Label dim(String t) {
        Label l = new Label(t); l.setStyle("-fx-font-size:12px;-fx-text-fill:" + TXT_DIM + ";"); return l;
    }
    ...
    private Label fitItemLbl(String t) {
        Label l = new Label(t); l.setStyle("-fx-font-size:13px;-fx-text-fill:" + TXT_CREAM + ";");
        l.setWrapText(true); return l;
    }
    private Label wrapLabel(String t) {
        Label l = new Label(t); l.setStyle("-fx-font-size:12px;-fx-text-fill:" + TXT_LIGHT + ";");
        l.setWrapText(true); return l;
    }
    private Label emptyLbl(String t) {
        Label l = new Label(t); l.setStyle("-fx-font-size:12px;-fx-text-fill:" + TXT_DIM + ";-fx-padding:16 8 0 8;");
        return l;
    }

    private TextField field(String prompt, double w) {
        TextField tf = new TextField(); tf.setPromptText(prompt); tf.setPrefWidth(w);
    ...
    private ComboBox<String> cb(String... items) {
        ComboBox<String> c = new ComboBox<>(); c.getItems().addAll(items); c.setValue(items[0]);
        c.setPrefWidth(175);
    ...
    private HBox row2(Node a, Node b) { HBox h = new HBox(12, a, b); h.setAlignment(Pos.CENTER_LEFT); return h; }
    private HBox hrow() { HBox h = new HBox(10); h.setPadding(new Insets(4)); h.setAlignment(Pos.CENTER_LEFT); return h; }
    ...
    private void dnd(StackPane zone, ImageView iv, Label hint, java.util.function.Consumer<String> setter) {
    ...
    private File pickImg() {
        FileChooser fc = new FileChooser(); fc.setTitle("Choose an image");
    ...
    private String buildSuggestion() {
        ...
        if (tips.isEmpty()) tips.add("• Select items in My Wardrobe for personalised suggestions");
        return String.join("\n", tips);
    }
    private String ratingWord(int s) {
        return switch(s){case 1->"Not great";case 2->"Okay";case 3->"Good";case 4->"Great!";default->"Perfect!";};
    }

    private String typeEmoji(String t) {
        return switch(t){case"top"->"👕";case"bottom"->"👖";case"shoes"->"👟";
                         case"outer"->"🧥";case"acc"->"👜";default->"👗";};
    }
    private String colHex(String c) {
        return switch(c.trim().toLowerCase()){
            case"red"->"#E74C3C";case"blue"->"#3498DB";case"black"->"#2C3E50";
            case"white"->"#ECF0F1";case"green"->"#27AE60";case"pink"->"#FF69B4";
            case"purple"->"#9B59B6";case"yellow"->"#F1C40F";case"orange"->"#E67E22";
            case"grey","gray"->"#95A5A6";case"navy"->"#1A252F";case"lavender"->"#DDA0DD";
            case"lime"->"#ADFF2F";case"brown"->"#8B4513";case"beige"->"#F5F0DC";
            default->"#BDC3C7";
        };
    }
